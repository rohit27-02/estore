    ...
    <div className="flex flex-wrap justify-evenly   -m-4">
      {Object.keys(products).map((item)=>{
       return <div key={products[item]._id} className="lg:w-1/5 md:w-1/3 p-2 m-4  w-full shadow-lg cursor-pointer ">
        ...
        <div className="mt-4 ">
          <h3 className="text-center text-gray-500 text-xs tracking-widest title-font mb-1">Hoodies</h3>
          ...
          <div className='mt-2'>
            ...
            {products[item].color.includes("orange") && <button className=" bg-orange-500 rounded-full w-5 h-5 focus:outline-none mx-1"></button>}
            </div>
        </div>
        </a>
      </div>})}
